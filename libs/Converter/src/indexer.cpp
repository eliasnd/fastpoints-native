...
void doIndexing(string targetDir, State& state, Options& options, Sampler& sampler, CancelCallback shouldCancel) {
	...
	TaskPool<Task> pool(numThreads, [&onNodeCompleted, &onNodeDiscarded, &writeAndUnload, &state, &options, &activeThreads, tStart, &lastReport, &totalPoints, totalBytes, &pointsProcessed, chunks, &indexer, &nodes, &mtx_nodes, &sampler, shouldCancel](auto task) {
		...
		sampler.sample(chunkRoot.get(), attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded, shouldCancel);
	...
	{ // process chunk roots in batches
		...
		for(auto& task : tasks){
			...
			sampler.sample(task.node, attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded, shouldCancel);
	...
	if (chunks->list.size() == 1) {
		...
		sampler.sample(indexer.root.get(), attributes, indexer.spacing, onNodeCompleted, onNodeDiscarded, shouldCancel);
	}
